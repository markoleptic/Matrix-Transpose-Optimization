...
n: number of columns
float* src: source matrix (m-by-n matrix)
rs_s >=1: row stride of source matrix
cs_s >= 1: column stride of source matrix
float* dst: destination matrix (n-by-m matrix)
rs_d >= 1: row stride of destination matix
cs_d >= 1: column stride of destination matix
constraint: if rs_s == 1 then column major ordering
constraint: if cs_s == 1 then row major ordering
constraint: This is an out-of-place transposition meaning src and dst WILL NOT OVERLAP.
